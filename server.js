...
Keywords that mean a mark follows: "got", "গাট", "marks", "নম্বর" , "peyece" , "পেয়েছে"
...
// Pipeline: JSON { audio: base64, assessment } → decode → Whisper → transcript → DeepSeek → JSON marks
// Response: [{"student id": "XXX-XX-XXX", "mark": 15}] or {"error": "..."}
app.post('/api/analyze-audio', async (req, res) => {
  const { audio: base64Audio, assessment } = req.body;
  ...
  // Decode base64 to binary buffer and write to temp file (Whisper reads from disk)
  const audioBuffer = Buffer.from(base64Audio, 'base64');
  const tempFile = path.join(AUDIO_DIR, `audio-${Date.now()}.webm`);
  fs.writeFileSync(tempFile, audioBuffer);
  console.log(`[Audio] Decoded and saved to: ${tempFile} (${audioBuffer.length} bytes)`);
